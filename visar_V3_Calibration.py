...
class CalibrationData:
    ...
    def map(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        """
        print(f'{self.visar.sweep_time}ns')
...
class VISAR:
    ...
    def format(self, compact=False):
        ...
        """
        self.run_str = f'p{self.proposal}, r{self.runNumber}'
    ...
    def sweep_delay(self):
        ...
        for key in ['actualDelay', 'actualPosition']:
            if key in self.trigger:
                print('test', self.cal.reference_trigger_delay)
                print('test2', self.trigger[key][by_id[self.shot()[1]]].ndarray())
    ...
    #     """
    #     scope_trace_V = self.run['HED_PLAYGROUND/SCOPE/TEXTRONIX_TEST:output', 'ch1.corrected']
    #     scope_trace_V = scope_trace_V[by_id[self.shot()[1]]].ndarray()
    #     scope_trace = np.vstack((scope_trace_V, scope_trace_V))
    #     print(scope_trace.shape)
        
    #     # return scope_trace
    #     digitizer_max = np.max(np.unique(scope_trace[:, :40000], axis = 1), axis = 1)
    #     print(digitizer_max)
    #     dipole_trace_idx = [np.where(st > dm)[0] for (st, dm) in zip(scope_trace, digitizer_max)]
    #     print(dipole_trace_idx)

    #     dt = 0.2 # 200 ps/sample
    #     time_axis_trace = [(np.arange(dti[0]-25, dti[-1]+25) - dti[0])*dt for dti in dipole_trace_idx]
    ...
    #     power_trace = [st[dti[0]-25:dti[-1]+25]*ps for (st, dti, ps) in zip(scope_trace, dipole_trace_idx, power_sacling)]
    ...
    def plot(self, ax=None):
        ...
        for idx, (shot_id, data, dipole_delay, dipole_energy, sweep_delay) in enumerate(zip(shot_ids, shot_data, self.dipole_delay, self.dipole_energy, self.sweep_delay)):
            ...
            print(self.cal.dipole_zero)
            print(time_conversion(self.cal.dipole_zero))
            print('dipole_delay', dipole_delay)
            print('sweep_delay', sweep_delay)
            time_axis -= offset
            ...
            im = ax.imshow(data, extent=[time_axis[0], time_axis[-1], space_axis[0], space_axis[-1]], cmap='jet', vmin=data[xs.min():xs.max(), ys.min():ys.max()].mean()-2*data[xs.min():xs.max(), ys.min():ys.max()].std(), vmax=data[xs.min():xs.max(), ys.min():ys.max()].mean()+2*data[xs.min():xs.max(), ys.min():ys.max()].std())
            ax.vlines(
                Xdelay,
                ymin=space_axis[0],
                ymax=space_axis[-1],
            ...
            # plt.savefig(f'./r_{runNumber}/{self.name}_{idx}.png')
    ...
    def save(self):
        ...
        ref_data, shot_data = self.frame()
        meta = self.run.run_metadata()
        # with h5py.File(f'VISAR_p{self.meta.get("proposalNumber", "?"):06}_r{self.meta.get("runNumber", "?"):04}.h5', 'a') as hdf:
        with h5py.File(f'VISAR_p{proposalID}_r{runNumber}.h5', 'a') as hdf:
...
class SOP:
    ...
    def __init__(self, run, cal_file=None):
        self.name = 'SOP'
    ...
    def plot(self, ax=None):
        ...
        for idx, (shot_id, data, dipole_delay, dipole_energy, sweep_delay) in enumerate(zip(shot_ids, shot_data, self.dipole_delay, self.dipole_energy, self.sweep_delay)):
            ...
            # im = ax.imshow(data, extent=[time_axis[0], time_axis[-1], space_axis[0], space_axis[-1]], cmap='jet', vmin=0, vmax=data.mean()+3*data.std())
            # ax.vlines(
            #     Xdelay,
            #     ymin=space_axis[0],
            #     ymax=space_axis[-1],
            ...
            # fig.tight_layout()
            # meta = self.run.run_metadata()
            # plt.savefig(f'./{self.name}_p{meta.get("proposalNumber", "?"):06}_r{meta.get("runNumber", "?"):04}_{idx}.png')
